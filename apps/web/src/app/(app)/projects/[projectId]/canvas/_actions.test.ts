...
const sendMock = vi.fn(
  async (_payload: { name: string; data: Record<string, unknown> }) => undefined,
);
...
const entityReturning = vi.fn(async () => [{ id: "c1", projectId: "p1", branchId: "b1" }]);
const findingsWhere = vi.fn(async () => undefined);

vi.mock("@/inngest/client", () => ({ inngest: { send: sendMock } }));

vi.mock("@lore/auth/permissions", () => ({
  requireOrgRole: vi.fn(async () => ({ success: true, data: { userId: "u1" } })),
}));
    ...
    db: {
      update: vi.fn(() => {
        ...
        if (updateCall === 1) {
          ...
          return { set: () => ({ where: () => ({ returning: entityReturning }) }) };
        }
      ...
      insert: vi.fn(() => ({
        values: () => ({
          returning: vi.fn(async () => [{ id: "c1", projectId: "p1", branchId: "b1" }]),
        }),
      })),
      select: vi.fn(),
    },
...
beforeEach(() => {
  vi.clearAllMocks();
  updateCall = 0;
  entityReturning.mockResolvedValue([{ id: "c1", projectId: "p1", branchId: "b1" }]);
});

describe("deleteEntity (Phase 10 wiring)", () => {
  it("sync-resolves related open findings AND emits entity.updated", async () => {
    const out = await deleteEntity({ type: "character", entityId: "c1", orgId: "o1" });
    ...
    const sent = sendMock.mock.calls[0]![0] as {
      name: string;
      data: { entityId: string; projectId: string; branchId: string; orgId: string };
    };
  ...
  it("does NOT emit when the entity is not found", async () => {
    ...
    const out = await deleteEntity({ type: "character", entityId: "missing", orgId: "o1" });
